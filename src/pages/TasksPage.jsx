...
function TaskCard({ task, onComplete, onPartial }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "16px" }}>
        <MetaChip icon="🗂" label={task.type} />
        <MetaChip icon="📅" label={task.date} />
...
function TimeRemaining({ createdAt }) {
  ...
  const remaining = 24 * 60 * 60 * 1000 - (Date.now() - created);
...
export default function TasksPage() {
  ...
  const getTasks = async () => {
    try {
      ...
      // 24 soatdan o'tgan vazifalarni avtomatik o'chirish
      const deletePromises = [];
      const activeTasks = [];

      snapshot.docs.forEach(d => {
        const data = d.data();
        // createdAt Firestore Timestamp yoki milliseconds bo'lishi mumkin
  ...
  const partialTask = async (id) => {
    await updateDoc(doc(db, "tasks", id), { status: "partial" });
    getTasks();
  };
